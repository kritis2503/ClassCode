...
//         return base;
//     }
//     allsol lp=allsolBST(root->left);
//     allsol rp=allsolBST(root->right);

//     allsol myAns;
//     if(lp.isBST && rp.isBST && lp.max>)
// }
...
void predessor(Node *node,int d,Node &*pre)
{
    if(node==nullptr)
        return ;
    if(node.data==d)
    ...
    predessor(node->left,d,node);
    predessor(node->right,d,node);
}
void sucessor(Node *node,int d,Node &*suc)
{
     if(node==nullptr)
        return ;
    if(node->data==d)
    ...
    sucessor(node->left,d,node);
    sucessor(node->right,d,node);
}
void ceilval(Node *node,int d,int ceil)
{
    if(node==nullptr)
    return 0;
    ceilval(node->left,d,ceil);
    ceilval(node->right,d,ceil);
    if(node->data>d)
    {
        if(node->data<ceil)
            ceil=node->data;
...
void floorval(Node *node,int d,int ceil)
{
    if(node==nullptr)
    return 0;
    floorval(node->left,d,ceil);
    floorval(node->right,d,ceil);
    if(node->data<d)
    {
        if(node->data>ceil)
            floor=node->data;
...
Node *pre=null;
Node *suc=null;
void BSTpresuc(Node *root,int d)
{
    Node* curr=root;
    while(curr!=null)
    {
...
void solve()
{
    ...
    display(root);
    Node *pre;
    predessor(root,80,nullptr);
    cout<<pre.data<<endl;
    Node *suc;
    sucessor(root,80,nullptr);
    cout<<suc.data<<endl;
