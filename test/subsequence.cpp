...
int combinationINFI_subseq(vector<int> &arr, int idx, int tar, string ans)
{
    ...
    int count = 0;
    if (tar - arr[idx] >= 0)
        count += combinationINFI_subseq(arr, i, tar - arr[idx], ans + to_string(arr[idx]) + " ");
    count += combinationINFI_subseq(arr, i, tar, ans);
    return count;
}
...
int permutationSingleCoin_subseq(vector<int> &arr, int idx, int tar, string ans)
{
    ...
    int count = 0;
    if (tar - arr[idx] >= 0)
        count += permutationSingleCoin_subseq(arr, 0, tar - arr[idx], ans + to_string(arr[idx]) + " ");
    count += permutationSingleCoin_subseq(arr, 0, tar, ans);
    return count;
}
